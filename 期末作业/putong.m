...
Ox=-1000;Oy=0;Oz=100;r=500;[x,y,z] = sphere(800);
O = surf(r*x+Ox, r*y+Oy, r*z+Oz);O.EdgeColor = [0.94,0.76,0.80];%山大淡粉色
hold on;
%% 计算路径上的点的坐标:分成竖直线、横直线、撇捺三种路径
% 普通山字：一横三竖。大：1横-1竖-1撇-1捺。
point_heng1=Hengxian(-15,15,15) %计算横线上点的坐标
point_heng2=Hengxian(-10,10,-5)%这里的角度值都可以任意自定义，可以画出任何位置和斜度的线
point_shu1=flip(Shuxian(15,30,0)) %计算竖线上点的坐标
point_shu2=flip(Shuxian(-10,5,0))%用flip倒转算出来的轨迹点的排列方向，方便下面画轨迹
point_shu3=flip(Shuxian(15,20,-15))
point_shu4=flip(Shuxian(15,20,15))
%计算撇捺上点的坐标
point_xie1=flip(Piena(-30,-28,-20,-13)) 
point_xie2=flip(Piena(-30,-28,20,13))%多段倾斜角度不同的直线拟合弧线
point_xie11=flip(Piena(-28,-23,-13,-7)) %这里一撇用了四段小直线去拟合弧线
point_xie22=flip(Piena(-28,-23,13,7))
point_xie111=flip(Piena(-23,-17,-7,-2)) 
point_xie222=flip(Piena(-23,-17,7,2))
point_xie1111=flip(Piena(-17,-10,-2,0)) 
point_xie2222=flip(Piena(-17,-10,2,0))

%拼接所有的轨迹点
points=[point_shu3;point_heng1;point_shu4;point_shu1;
point_heng2;point_shu2;point_xie1111;point_xie111;point_xie11;point_xie1;
point_xie2222;point_xie222;point_xie22;point_xie2;]
view(90,1)%面向轨迹正面视角
% view([1,1,1])
%plot3的颜色标记这里指定的是'ro'，是实心圆点，所以会有点的凸起，改成'r-'就是红色的线
plot3(points(:,1),points(:,2),points(:,3),'ro','linewidth',3);hold on;
%% 定义机械臂
...
jxb.qlim=[-pi,pi;-pi,pi;-pi,pi;-pi,pi;-pi,pi;-pi,pi;]%限制关节转角为-180到180
